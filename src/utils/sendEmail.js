...
export function buildEmailFromType(type, context = {}) {
  switch (type) {
    ...
    // `verifyToken` is a random-hash token generated by the caller (see
    // sendVerificationEmail in server.js) and stored hashed in
    // email_verification_tokens — not a JWT, so it can be revoked/expired
    // from the DB the same way password reset tokens are.
    case "verify_email":
      return {
        subject: "Verify Your Email Address",
        body: `Hi ${context.username}, please confirm this is your email address: ${FRONTEND_URL}/verify-email?token=${context.verifyToken}\n\nThis link expires in 24 hours. If you didn't request this, you can safely ignore this email.`,
